...
class Account
{
    ...
    public:
        ...
        void Account_Details(Account accArr[],int& numOfAccounts);
        void print();
    ...
    do {
        ...
        for (int i = 0; i < numOfAccounts; i++) {
            if (account_no == accArr[i].getAccountNumber())
            {
                unique = false;
        ...
        if (!unique) 
        {
            break; // exit the outer do-while loop if the account number already exists
        }
    } while (!unique);
        do
        {
...
void bankingServices(/*Account *accArr, int& numOfAccounts*/)
{
    ...
    switch (service)
    {
        case 1: 
        {
            ...
            acc->Account_Details(accArr, numOfAccounts);
